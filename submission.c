...
#define PM_SNAPSHOT_TEXT_CAP 8192
...
static pm_pcb_t *find_zombie_child(pm_pcb_t *parent, int child_pid) {
    if (child_pid == PM_ANY_CHILD) {
        ...
        for (int i = 0; i < child_count; i++) {
            // Bounds check - array might shrink during iteration
            if (i >= parent->child_count) break;
            if (parent->children[i] == 0) continue;
...
int pm_exit(int pid, int status) {
    ...
    if (parent) {
        // Broadcast to wake ALL waiting threads; they will re-check conditions
...
int pm_wait(int parent_pid, int child_pid, int *reaped_pid, int *exit_status) {
    ...
    for (;;) {
        ...
        if (child) {
            // Found zombie - reap it and return immediately
...
int main(int argc, char **argv) {
    ...
    if (pm_init("snapshots.txt") != 0) {
        fprintf(stderr, "Failed to initialize process manager\n");
        return 1;
    }

    pthread_t monitor_thread;
    if (pthread_create(&monitor_thread, NULL, pm_monitor_thread, NULL) != 0) {
    ...
    pthread_t *worker_threads = calloc((size_t)worker_count, sizeof(*worker_threads));
    pm_worker_args_t *worker_args = calloc((size_t)worker_count, sizeof(*worker_args));
    ...
    for (int i = 0; i < worker_count; i++) {
        ...
        if (pthread_create(&worker_threads[i], NULL, pm_worker_thread, &worker_args[i]) != 0) {
